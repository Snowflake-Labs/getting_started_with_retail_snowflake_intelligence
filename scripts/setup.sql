/*--
 ...
 (MODIFIED TO USE CURRENT DATE FOR DATA GENERATION)
--*/
...
def main(session):
	...
	# "Current" date for reference (MODIFIED TO USE TODAY'S DATE)
	...
	# Ensure the most recent scents are still recent relative to the new current date
	launch_dates["Lemon Verbena Grove"] = current_date - timedelta(days=current_date.toordinal() - date(2025, 6, 17).toordinal()) # Preserves gap
	launch_dates["Peachwood Breeze"] = current_date - timedelta(days=current_date.toordinal() - date(2025, 6, 17).toordinal()) # Preserves gap
	# Note: A simpler approach for the quickstart context is to re-assign:
	# launch_dates["Lemon Verbena Grove"] = current_date - timedelta(days=3)
	# launch_dates["Peachwood Breeze"] = current_date - timedelta(days=3)
	# But using the current logic maintains the *relative* timing as originally intended.
...
def main(session):
	...
	# Current date and launch dates (MODIFIED TO USE TODAY'S DATE)
	current_date = date.today()
	two_years_ago = current_date - timedelta(days=2*365)
	...
	# Ensure the most recent scents are still recent relative to the new current date
	launch_dates["Lemon Verbena Grove"] = current_date - timedelta(days=current_date.toordinal() - date(2025, 6, 17).toordinal())
	launch_dates["Peachwood Breeze"] = current_date - timedelta(days=current_date.toordinal() - date(2025, 6, 17).toordinal())
...
-- Generate initial inventory based on sales averages (MODIFIED TO USE CURRENT_DATE())
...
SELECT product_name,	
	...
	current_date() as inventory_as_of_date
